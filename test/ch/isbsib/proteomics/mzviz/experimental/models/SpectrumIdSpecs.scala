  ...
  "case" should {
    "need no new" in {
      val spid = SpectrumRef(Some(ScanNumber(12)),
        ExpPeakPrecursor(moz = Moz(123.45), intensity=Intensity(678), retentionTime=RetentionTime(123.45), charge=Charge(2), None, molecularMass=Some(MolecularMass(244.88544))), "no title", SpectrumId(SpectrumUniqueId("12"), RunId("Paf")))
