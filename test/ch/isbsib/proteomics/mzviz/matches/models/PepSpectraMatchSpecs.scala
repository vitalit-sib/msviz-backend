...
class PepSpectraMatchSpecs extends Specification {
  "pepSpectraMatch" should {
    ...
    val sequence = "AKKKAA"
    val oneModifRef = ModificationRef(ModifName("Phospho"), ModifSource("UNIMOD"))
    val modificationRefs = Vector(Nil, Seq(oneModifRef), Nil, Nil, Nil, Nil)
    val pep = Peptide(sequence = sequence, molMass = 123.23, modificationRefs = modificationRefs)
