...
class LoaderMzIdentSpecs extends Specification {
  ...
  "parseSearchDbSourceInfo" should {
    ...
    "parse M_100" should {
        ...
        "check peptide match info" in {
          ...
          psm(0).matchInfo.correctedMoz must equalTo(None)
          psm(0).matchInfo.correctedMolMass must equalTo(None)
  ...
  "parse MzId with equal scores" should {
    ...
    "check if we find TLTLVDTGIGMTK" in {
      psmsFlt.size mustEqual(1152)
    }

    "check if we find TLTLVDTGIGMTK as first rank" in {
      psmsFlt(0).matchInfo.rank mustEqual(Some(1))
    }
